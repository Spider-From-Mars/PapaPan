...
PlotComponent::PlotComponent(PanCakeAudioProcessor &p)
    : audioProcessor(p), mod(p.getPanner().getModulation()),
      ...
      triangleButton("Triangle", juce::DrawableButton::ImageFitted)
{
    ...
    auto sineSVG =
        juce::Drawable::createFromImageData(BinaryData::sinWave_svg, BinaryData::sinWave_svgSize);
    ...
    visualiser.setColours(juce::Colours::transparentWhite, BaseColours::visualiserWaveColor);
...
void PlotComponent::paint(juce::Graphics &g)
{
    ...
    const int usefulHeight = bounds.getHeight() - 2 * textFieldHeight;
    ...
    const int channelLabelY = textFieldHeight - channelLabelPad;

    g.setColour(BaseColours::transparentPink);
    g.fillRect(0, 0, bounds.getWidth(), textFieldHeight);
    g.fillRect(0, bounds.getBottom() - textFieldHeight - 2, bounds.getWidth(), textFieldHeight + 2);

    g.setColour(BaseColours::white);
    g.drawSingleLineText("Left", channelLabelPad, channelLabelY);

    auto labelWidth = juce::GlyphArrangement::getStringWidth(g.getCurrentFont(), "Right");
    g.drawSingleLineText("Right", bounds.getWidth() - labelWidth - channelLabelPad, channelLabelY);
}
...
void PlotComponent::resized()
{
    ...
    sinButton.setBounds(10, bounds.getBottom() - buttonWidth, buttonWidth, buttonWidth);
    triangleButton.setBounds(sinButton.getX() + buttonWidth + 5, sinButton.getY(), buttonWidth,
                             buttonWidth);

    visualiser.setBounds(bounds.getCentreX(), bounds.getCentreY(), bounds.getHeight(), 90);
    visualiser.setTransform(
        juce::AffineTransform::rotation(-juce::MathConstants<float>::halfPi, bounds.getCentreX(),
                                        bounds.getCentreY())
...
void PlotComponent::drawPlot(juce::Graphics &g, int xSize) const
{
    ...
    g.strokePath(plotPath, juce::PathStrokeType(4, juce::PathStrokeType::curved,
...
void PlotComponent::drawPanSegment(juce::Graphics &g, int xSize) const
{
    ...
    {
        segmentPath.lineTo(centerY + waveFunction(p) * amplitude, offset + (p / twoPi) * xSize);
    }

    g.strokePath(segmentPath, juce::PathStrokeType(4, juce::PathStrokeType::curved,
                                                   juce::PathStrokeType::rounded));
}
...
void PlotComponent::drawGrid(juce::Graphics &g, int x, int y, int height, int linesNum) const
{
    ...
    for (int i = 0; i <= horizontaLinesNum; i++)
    {
        ...
        g.fillRect(juce::Rectangle<float>(x, y + i * lineStep, getWidth(), lineThickness));
    ...
    {
        g.fillRect(juce::Rectangle<float>(x + i * lineStep, 0, lineThickness, getHeight()));
    }
...
void PlotComponent::setupWaveButton(juce::Button &button, juce::AudioParameterChoice *waveParam,
                                    Panner::waveType type)
{
    button.setColour(juce::DrawableButton::backgroundColourId, juce::Colours::transparentBlack);
    button.setColour(juce::DrawableButton::backgroundOnColourId, BaseColours::darkPink);
