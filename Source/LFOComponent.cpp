    ...
    : lfoSlider(*apvts.getParameter("HERTZRATE"), "HZ")
{
    ...
    lfoSliderAttachment = std::make_unique<juce::AudioProcessorValueTreeState::SliderAttachment>(
        apvts, "HERTZRATE", lfoSlider);
    modeSelectorAttachment =
        std::make_unique<juce::AudioProcessorValueTreeState::ComboBoxAttachment>(apvts, "MODE",
                                                                                 modeSelector);
    durationSelectorAttachment =
        std::make_unique<juce::AudioProcessorValueTreeState::ComboBoxAttachment>(apvts, "DURATION",
                                                                                 durationSelector);
...
void LFOComponent::resized()
{
    ...
    durationSelector.setBounds(modeSelector.getX() + durationSelectorWidth / 2,
                               modeSelector.getBottom() + 10, durationSelectorWidth,
                               modeSelector.getHeight());
}

void LFOComponent::setSelectors(juce::AudioProcessorValueTreeState &apvts)
{
    juce::AudioParameterChoice *mode =
        dynamic_cast<juce::AudioParameterChoice *>(apvts.getParameter("MODE"));
    juce::StringArray modes = mode->choices;
    for (int i = 0; i < modes.size(); ++i)
    {
        modeSelector.addItem(modes[i], i + 1);
        switch (i)
        {
        case 1:
        case 3:
    ...
    juce::AudioParameterChoice *duration =
        dynamic_cast<juce::AudioParameterChoice *>(apvts.getParameter("DURATION"));
