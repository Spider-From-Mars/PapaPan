...
class Modulation
{
  public:
    ...
    void prepare(juce::dsp::ProcessSpec &spec) { sampleRate = spec.sampleRate; }

    void advance();

    void hertzRetrigProcess(float hertzRate);
    void beatRetrigProcess(double duration, double bpm);
    void hertzSyncedProcess(float hertzRate, const juce::AudioPlayHead::PositionInfo &posInfo);
    void beatSyncedProcess(double duration, const juce::AudioPlayHead::PositionInfo &posInfo);
...
class Panner
{
  ...
  private:
    ...
    std::array<double, 18> noteDurations = {
        32.0,    16.0,    8.0,     4.0,      3.0,     2.0,      1.5,      1.0,      3.0 / 4,
        1.0 / 2, 3.0 / 8, 1.0 / 4, 3.0 / 16, 1.0 / 8, 1.0 / 16, 1.0 / 32, 1.0 / 64, 1.0 / 128};
